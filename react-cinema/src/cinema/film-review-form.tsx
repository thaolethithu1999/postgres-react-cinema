...
export const ReviewFilm = ({ film}: Props) => {
  ...
  const load = async () => {
    ...
    const { id } = params;
    filmRateSM.filmId = id;

    const searchResult = await FilmRateService.search(filmRateSM);
  ...
  const moreReview = async (e: any) => {
    ...
    const searchRates = await FilmRateService.search(filmRateSM);
  ...
  const postReview = async (data: DataPostRate): Promise<void> => {
    try {
      ...
      filmRate.filmId = film.filmId;
      filmRate.userId = id;
      filmRate.rate = data.rate;
      filmRate.review = data.review;
      await filmService.rateFilm(filmRate);
