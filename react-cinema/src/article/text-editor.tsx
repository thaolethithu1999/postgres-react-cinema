...
export const TextEditorComponent: React.FC<{
  ...
  const selectionChange = (e: MouseEvent) => {
    if (window.getSelection() && document.createRange()) {
      const changed = saveSelectionForCreateRanage(document.getElementById('content-editor'));
      setSelectionCreateRange(true);
      setSavedSelection(changed);
    } else {
      // @ts-ignore
      if (document.selection && document.body.createTextRange) {
        const changed = saveSelectionForCreateTextRange(document.getElementById('content-editor'));
  ...
  const findAndActiveSelected = () => {
    ...
    for (const _node: Node of parentNodesCurrent) {
      ...
      // @ts-ignore
      if (formatBackgroundColor.length > 0 && document.getElementById('FormatBackgroundColor')) {
        // @ts-ignore
        document.getElementById('FormatBackgroundColor').value = formatBackgroundColor[0];
      }
          ...
          <div id='toolBar' className={''}>
            {!showToolbar ? <div className={'toolBar btn-group toolBar-button'}>
              <div className={'input-group'}>
                <select name='FormatText' id='FormatText' onChange={($event) => {
                ...
                <select name='FormatFont' id='FormatFont' onChange={($event) => {
                ...
                <select name='FormatSize' id='FormatSize' onChange={($event) => {
                ...
                <select name='FormatColor' id='FormatColor' onChange={($event) => {
                ...
                <div className='input md-3'>
                  <select name='FormatBackgroundColor' id='FormatBackgroundColor' onChange={($event) => {
