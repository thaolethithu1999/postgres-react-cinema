...
export class RateManager extends Manager<Rate, RateId, RateFilter> implements RateService {
  constructor(search: Search<Rate, RateFilter>, public replySearch: Search<Reply, ReplyFilter>,
...
export function useRateService(db: DB, mapper?: TemplateMap): RateService {
  ...
  const queryReply = useQuery('reply', mapper, replyModel, true);
  const builderReply = new SearchBuilder<Reply, ReplyFilter>(db.query, 'reply', replyModel, db.driver, queryReply);
  const replyRepository = new SqlReplyRepository(db, 'reply', buildToSave);
  return new RateManager(builder.search, builderReply.search, repository, infoRepository, replyRepository, rateReactionRepository);
}
